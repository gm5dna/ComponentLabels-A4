...
class Optocoupler(BasicComponent):
    def __init__(self, name: str | Label, channels: str, isolation_voltage: str, speed: str):
        self.value = name
        self.type = "optocoupler"
        self.channels = channels
        self.str1 = "{}".format(isolation_voltage)
        self.str2 = "{}".format(speed)
        self.str3 = None
    ...
    def draw_icon(self, c: Canvas, x: float, y: float, size: float) -> None:
        c.rect(x - size * 2 / 3, y - size * 2 / 3, size * 4 / 3, size * 4 / 3, fill=0, stroke=1)

        c.line(x - size, y, x - size / 2, y)
        c.line(x + size / 2, y + size / 3, x + size, y + size / 3)
        c.line(x + size / 2, y - size / 3, x + size, y - size / 3)

        c.line(x - size / 2, y - size / 4, x - size / 6, y)
        c.line(x - size / 2, y + size / 4, x - size / 6, y)
        c.line(x - size / 2, y - size / 4, x - size / 2, y + size / 4)

        c.line(x + size / 4, y - size / 3, x + size / 4, y + size / 3)
        c.line(x + size / 4, y, x + size / 2, y + size / 3)
        c.line(x + size / 4, y, x + size / 2, y - size / 3)
        ...
        self.draw_arrow(c, x - size / 12, y + size / 8, size / 3, size / 6, atan(1))
        self.draw_arrow(c, x + size / 12, y - size / 8, size / 3, size / 6, atan(1))
